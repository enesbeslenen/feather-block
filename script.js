...
const LAYOUT_BOARD_ZONE = 0.58;
const LAYOUT_TRAY_ZONE = 0.22;
const LAYOUT_BOTTOM_PAD = 0.06;
const LAYOUT_BOARD_TRAY_GAP = 0.04;
...
/**
 * Block Blast tarzı yerleşim: üstte ızgara (~%58), altta tepsi (~%22).
 */
function getLayoutMetrics(width, height) {
  const sidePad = Math.max(10, width * 0.05);
  const innerW = width - sidePad * 2;

  const bottomPad = height * LAYOUT_BOTTOM_PAD;
  const trayHeight = height * LAYOUT_TRAY_ZONE;
  const boardTrayGap = height * LAYOUT_BOARD_TRAY_GAP;
  const boardZoneH = height * LAYOUT_BOARD_ZONE;

  let boardSide = Math.min(innerW, boardZoneH);
  const boardX = (width - boardSide) / 2;
  const boardY = (boardZoneH - boardSide) / 2;

  const trayW = innerW;
  const trayX = (width - trayW) / 2;
  const trayY = height - bottomPad - trayHeight;
